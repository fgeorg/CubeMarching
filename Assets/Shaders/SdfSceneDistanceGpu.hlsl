...
struct SdfPrimitive {
    ...
    float4 color;       // rgba
    float4 material;    // x=metallic, y=smoothness
};
...
SdfMaterial LerpMaterial(SdfMaterial a, SdfMaterial b, float t) {
    SdfMaterial r;
    r.color      = lerp(a.color,      b.color,      t);
    r.metallic   = lerp(a.metallic,   b.metallic,   t);
    r.smoothness = lerp(a.smoothness, b.smoothness, t);
    return r;
}

// Returns material by evaluating the full postfix operator tree with operator-aware blending.
...
SdfMaterial GetMaterialAtScene(float3 p) {
    SdfStack stack = (SdfStack)0;
    ...
    for (int i = 0; i < _SdfNodeCount; i++) {
        ...
        if (t < SDF_PRIMITIVES_END) { // primitive — push
            ...
            if (sp < STACK_SIZE) {
                SetStackValue(stack, sp, d);
                SdfMaterial prim_mat;
                prim_mat.color      = _SdfPrimitives[node.primitiveIndex].color;
                prim_mat.metallic   = _SdfPrimitives[node.primitiveIndex].material.x;
                prim_mat.smoothness = _SdfPrimitives[node.primitiveIndex].material.y;
                SetMaterialStackValue(matStack, sp, prim_mat);
                sp++;
            }
        }
        else if (t <= SDF_BINARY_OPS_END && sp >= 2) { // binary operator — pop two, push result
            float b_d = GetStackValue(stack, --sp);
            SdfMaterial b_mat = GetMaterialStackValue(matStack, sp);
            float a_d = GetStackValue(stack, --sp);
            SdfMaterial a_mat = GetMaterialStackValue(matStack, sp);
            float r_d;
            SdfMaterial r_mat;
            if (t == SDF_UNION) {
                r_d = min(a_d, b_d);
                ...
                r_mat = LerpMaterial(b_mat, a_mat, h);
            }
            else if (t == SDF_INTERSECT) {
                r_d = max(a_d, b_d);
                if (a_d > b_d) {
    ...
    if (sp > 0) return GetMaterialStackValue(matStack, 0);
    SdfMaterial def = (SdfMaterial)0;
    def.color = float4(1, 1, 1, 1);
    return def;
}
